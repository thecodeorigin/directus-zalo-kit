...
export class ZaloService {
  ...
  public async apiSendMessageWithBroadcast(data: {
    ...
    try {
      ...
      if (!threadId) {
        throw new Error(`Could not determine Zalo threadId from conversationId: ${conversationId}`)
      }
  ...
  // =================================================================
  // == START OF PATCHED FUNCTION
  // =================================================================

  private async listenerHandleMessage(rawData: any) {
    // ✅ FIX: Định nghĩa conversationId bên ngoài khối try
    // để khối catch có thể truy cập
    ...
    try {
      ...
      if (existingMessages.length > 0) {
        console.log('[ZaloService] Message already exists:', messageId)
        return
      }
      ...
      // ✅ Xác định xem có phải group message không
      ...
      // Check conversation có phải là group không
        const existingConv = await this.dbFindConversation(recipientId)
        if (existingConv && existingConv.group_id) {
          isGroupMessage = true
          groupId = existingConv.group_id
        }
      }

      // ✅ Parse content và attachments
      let content = ''
      let attachments: any[] = []
      ...
      if (typeof rawData.content === 'string') {
        ...
        // Handle quick messages/commands
      ...
      // ✅ FIX 3: Xác định conversationId chính xác
      if (isGroupMessage && groupId && senderId) {
        // ID cuộc trò chuyện là *view* của người gửi trong group chat
        conversationId = `group_${groupId}_user_${senderId}`
      }
        ...
        console.error('[ZaloService] Cannot determine conversationId, message will fail.')
        return
      }

      // Tạo user người gửi (luôn luôn)
      ...
      if (isGroupMessage && groupId) {
        ...
        await this.dbUpsertGroup(groupId, {
          name: rawData.groupName || `Group ${groupId}`,
        })
        // Tạo conversation cho sender trong group
        await this.dbEnsureGroupConversation(groupId, senderId, true)
      }

      // ✅ FIX 2: Xử lý conversation (và user người nhận) TRƯỚC
      if (!isGroupMessage) {
        // ✅ FIX 2: Tạo user người nhận TRƯỚC
        if (recipientId && recipientId !== senderId) {
          await this.dbFetchAndUpsertUser(recipientId)
        }

        // ✅ FIX 2: Bây giờ mới upsert conversation
        await this.dbUpsertConversation(conversationId, rawData, senderId, recipientId)
      }

      // ✅ FIX 1: Bắt đầu sync SAU KHI conversation đã được tạo
      ...
      // ✅ FIX 4: Xử lý lỗi Race Condition (RECORD_NOT_UNIQUE)
      try {
        await messagesService.createOne(messageData)
      }
      catch (createError: any) {
        // Kiểm tra xem có phải lỗi "unique constraint" không
        if (createError.code === 'RECORD_NOT_UNIQUE') {
          console.warn(`[ZaloService] Race condition: Message ${messageId} already created. Ignoring duplicate.`)
          // Bỏ qua tin nhắn trùng lặp này
          return
        }

        // Nếu là lỗi khác, ném ra để khối catch bên ngoài xử lý
        throw createError
      }
      ...
      // ✅ Auto-apply labels (nếu không phải command)
      ...
      console.log(`[ZaloService] Message processed: ${messageId}`)
    }
    catch (error: any) {
      console.error('[ZaloService] Error handling message:', error)

      // ✅ FIX 3: Sử dụng conversationId đã tính toán trong khối try
      if (conversationId) {
        await this.syncFail(conversationId, error)
      }
      else {
        // Dự phòng nếu conversationId chưa được set
        const fallbackId = rawData.groupId || `direct_${rawData.uidFrom}_${rawData.idTo}`
        console.error(`[ZaloService] SyncFail using fallback ID: ${fallbackId}`)
        // Dòng này vẫn có thể fail nếu fallbackId không tồn tại,
        // nhưng đây là trường hợp tốt nhất có thể làm
        await this.syncFail(fallbackId, error)
      }
    }
  }
  ...
  private async dbEnsureGroupMember(
    ...
    try {
      ...
      // Update if status changed
        if (existing[0].is_active !== isActive) {
          await membersService.updateOne(existing[0].id, {
            is_active: isActive,
            joined_at: isActive ? new Date().toISOString() : existing[0].joined_at,
            left_at: isActive ? null : new Date().toISOString(),
          })
        }
      ...
      console.log(`[ZaloService] Added member ${userId} to group ${groupId}`)
    }
    catch (error: any) {
      console.error('[ZaloService] Error ensuring group member:', error)
      throw error
    }
  }

  private async dbEnsureGroupConversation(
    groupId: string,
    userId: string,
    isActive: boolean = true,
  ) {
    try {
      ...
      // Tìm conversation của user trong group này
      const existing = await conversationsService.readByQuery({
        filter: {
          _and: [
            { group_id: { _eq: groupId } },
            { participant_id: { _eq: userId } },
          ],
        },
        limit: 1,
      })
      ...
      if (existing.length > 0) {
      // Nếu đã có, update is_active nếu cần
        if (existing[0].is_active !== isActive) {
      ...
      // Tạo conversation mới cho member trong group
      ...
      console.error('[ZaloService] Error ensuring group conversation:', error)
      throw error
    }
  }
  ...
  private async dbUpsertConversation(
    ...
    try {
      ...
      const groupId = isGroup ? (rawData.groupId || conversationId) : null

      // ✅ Nếu là group message
      if (isGroup && senderId) {
      // Tạo conversation cho sender trong group (nếu chưa có)
        await this.dbEnsureGroupConversation(groupId!, senderId, true)
        return
      }
  ...
  public async dbGetGroupMembers(groupId: string, activeOnly: boolean = true) {
    try {
      ...
      return await membersService.readByQuery({
        filter,
        fields: ['id', 'group_id', 'owner_id', 'is_active', 'joined_at', 'left_at'],
        sort: ['joined_at'],
      })
    }
    catch (error: any) {
      console.error('[ZaloService] Error getting group members:', error)
      return []
    }
  }
  ...
  public async dbGetUserGroups(userId: string, activeOnly: boolean = true) {
    try {
      ...
      return await membersService.readByQuery({
        filter,
        fields: ['id', 'group_id', 'owner_id', 'is_active', 'joined_at'],
        sort: ['-joined_at'],
      })
    }
    catch (error: any) {
      console.error('[ZaloService] Error getting user groups:', error)
      return []
    }
  }
  ...
  public async dbMarkMemberLeft(groupId: string, userId: string) {
    try {
      ...
      console.error('[ZaloService] Error marking member left:', error)
      throw error
    }
  }
  ...
  public async dbMarkMemberRejoined(groupId: string, userId: string) {
    try {
      ...
      // Create new if doesn't exist
        await this.dbEnsureGroupMember(groupId, userId, true)
      }
    }
    catch (error: any) {
      console.error('[ZaloService] Error marking member rejoined:', error)
      throw error
    }
  }

  public async dbIsActiveMember(groupId: string, userId: string): Promise<boolean> {
    try {
      const conversationsService = await this.getService('zalo_conversations')
      ...
      console.error('[ZaloService] Error checking member status:', error)
      return false
    }
  }
   ...
   */
  private async dbUpsertGroup(groupId: string, groupInfo: any) {
    try {
      ...
      if (existing.length === 0) {
        ...
        console.log(`[ZaloService] Created group: ${groupId}`)
      }
      else {
        ...
        console.log(`[ZaloService] Updated group: ${groupId}`)
      }
    }
    catch (error: any) {
  ...
  public async syncGroups() {
    try {
      ...
      if (!response || !response.gridVerMap) {
        console.warn('[ZaloService] No groups data from API')
        return
      }
      ...
      console.log(`[ZaloService] Found ${groupIds.length} groups to sync`)

      const BATCH_SIZE = 20
      const currentUserId = this.api.getOwnId()

      if (!currentUserId) {
        console.error('[ZaloService] Cannot get current user ID for group sync')
        return
      }
      ...
      for (let i = 0; i < groupIds.length; i += BATCH_SIZE) {
        ...
        const batchPromises = batch.map(async (groupId) => {
          ...
          // 1. Get group info
            ...
            if (!groupInfo) {
              console.warn(`[ZaloService] No group info for ${groupId}`)
              return
            }
            ...
            if (groupInfo.creatorId) {
              try {
                await this.dbFetchAndUpsertUser(groupInfo.creatorId)
              }
              catch (ownerError: any) {
                console.warn(`[ZaloService] Failed to fetch owner ${groupInfo.creatorId}`)
                groupInfo.creatorId = null
              }
            }

            // 3. Upsert group
            await this.dbUpsertGroup(groupId, groupInfo)

            // 4. ✅ FIX: Sync group members
            await this.syncGroupMembers(groupId)

            // 5. Create conversation for current user (optional - for UI)
            const conversationsService = await this.getService('zalo_conversations')
            ...
            const existing = await conversationsService.readByQuery({
              filter: {
                _and: [
                  { group_id: { _eq: groupId } },
                  { participant_id: { _eq: currentUserId } },
                ],
              },
              limit: 1,
            })
            ...
            if (existing.length === 0) {
              const conversationId = `group_${groupId}_user_${currentUserId}`
              await conversationsService.createOne({
                id: conversationId,
                group_id: groupId,
      ...
      console.error('[ZaloService] Error in syncGroups:', error.message)
    }
  }

  public async syncGroupMembers(groupId: string) {
    try {
      if (!this.api) {
        throw new Error('Zalo API not initialized')
      }

      // ✅ Dùng getGroupInfo thay vì getGroupMembers (có thể không tồn tại)
      const response = await this.api.getGroupInfo?.(groupId)
      const groupInfo = response?.gridInfoMap?.[groupId]
      ...
      // ✅ Parse members từ memVerList
      // Format: ["userId1:version1", "userId2:version2", ...]
      if (!groupInfo.memVerList || !Array.isArray(groupInfo.memVerList)) {
        console.warn(`[ZaloService] No members list for group ${groupId}`)
        return
      }

      const members = groupInfo.memVerList
        .map((memVer: string) => {
          const [userId] = memVer.split(':')
          return { userId }
        })
        .filter((m: any) => m.userId)

      console.log(`[ZaloService] Syncing ${members.length} members for group ${groupId}`)

      for (const member of members) {
        const userId = member.userId

        try {
        // Fetch and upsert user
          await this.dbFetchAndUpsertUser(userId)

          // Add to group members table
          await this.dbEnsureGroupMember(groupId, userId, true)
        }
        catch (memberError: any) {
          console.error(`[ZaloService] Error syncing member ${userId}:`, memberError.message)
        // Continue with next member
        }
      }

      console.log(`[ZaloService] ✅ Synced ${members.length} members for group ${groupId}`)
    }
    catch (error: any) {
      console.error('[ZaloService] Error syncing group members:', error)
      throw error
    }
  }
